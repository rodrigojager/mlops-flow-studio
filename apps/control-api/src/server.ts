...
import { createControlApiSecurity, parseAllowedOrigins } from "./security.ts";
import {
  fastApiObservedEndpoints,
  flaskObservedEndpoints,
  starletteObservedEndpoints,
  djangoObservedEndpoints,
  expressObservedEndpoints,
  fastifyObservedEndpoints,
  koaObservedEndpoints,
  honoObservedEndpoints,
  nestJsObservedEndpoints,
  nextJsObservedEndpoints,
  isNextJsApiRouteCandidate,
  nextJsRoutePath,
  legacyHttpObservedEndpoints,
  grpcObservedEndpoints,
  goObservedEndpoints,
  rubyObservedEndpoints,
  javaObservedEndpoints,
  dotnetObservedEndpoints,
  phpObservedEndpoints,
  uniqueObservedEndpoints,
} from "./static-endpoints.ts";
...
interface SaveProjectBundleBody {
  project?: unknown;
  pipeline?: unknown;
}

interface ProjectTemplateDescriptor {
  id: string;
  name: string;
  description: string;
  category: string;
...
export function buildApp(options: BuildAppOptions = {}): FastifyInstance {
  ...
  const security = createControlApiSecurity({
    authToken: options.authToken ?? process.env.MLOPS_STUDIO_API_TOKEN,
    allowInsecureNoAuth: options.allowInsecureNoAuth,
    allowedOrigins: options.allowedOrigins ?? parseAllowedOrigins(process.env.MLOPS_STUDIO_ALLOWED_ORIGINS),
  });
  ...
  app.addHook("onRequest", async (request, reply) => {
    ...
    if (origin) {
      if (!security.isOriginAllowed(origin)) {
        return reply.status(403).send({
          ...
          message: "A origem da requisição não está autorizada para acessar a Control API.",
        });
      }
    ...
    if (!security.isAuthorized(request.headers.authorization)) {
      reply.header("WWW-Authenticate", "Bearer");
      return reply.status(401).send({
        error: "unauthorized",
        message: "Informe um Bearer token válido para acessar a Control API.",
      });
    }
  });
  ...
  app.get("/templates", async () => ({
    templates: await listProjectTemplates(workspaceRoot),
  }));
  ...
  app.put<{ Params: ProjectParams; Body: SaveProjectBundleBody }>("/projects/:projectId/bundle", async (request) => {
    return saveProjectBundle(workspaceRoot, request.params.projectId, request.body ?? {});
  });
  ...
  app.post<{ Body: RuntimeDockerBody }>("/runtime/docker/smoke", async (request) => {
    const runtimeApiKey = request.body?.outDir
      ? (await ensureRuntimeSecrets(workspaceRoot, request.body.outDir)).runtimeApiKey
      : optionalBodyString(request.body?.runtimeApiKey, "runtimeApiKey");
    return smokeRuntime(request.body?.baseUrl, request.body?.payload, request.body?.timeoutMs, runtimeApiKey);
  });
...
export function safeResolve(workspaceRoot: string, targetPath: string): string {
  ...
  assertNoSymlinkEscape(root, resolved, `Caminho fora do workspace por link simbólico: ${targetPath}`);
  return resolved;
}

function assertNoSymlinkEscape(rootPath: string, resolvedPath: string, message: string): void {
  ...
  const relative = path.relative(canonicalRoot, canonicalAncestor);
...
async function listProjectTemplates(workspaceRoot: string) {
  const templatesRoot = safeResolve(workspaceRoot, "templates");
  let entries;
  try {
  ...
  for (const entry of entries) {
    ...
    try {
      const template = await loadProjectTemplate(workspaceRoot, entry.name);
      templates.push({
        id: template.id,
        name: template.name,
        description: template.description,
        category: template.category,
        validationCase: template.validationCase,
      });
    } catch {
      continue;
    }
...
async function loadProjectTemplate(workspaceRoot: string, templateId: string): Promise<ProjectTemplateDescriptor> {
  const id = normalizeProjectId(templateId, "templateId");
  const templateDir = safeResolve(workspaceRoot, `templates/${id}`);
  const templatePath = path.join(templateDir, "template.yaml");
  let raw: unknown;
  try {
    raw = YAML.parse(await readFile(templatePath, "utf-8"));
  } catch (error) {
    throw new WorkspaceError(`Template não encontrado ou inválido: ${id}`, 404, error);
  }
  if (!isRecord(raw) || !isRecord(raw.instantiates)) {
    throw new WorkspaceError(`Template ${id} não declara instantiates.`, 422);
  }
  const projectRef = requiredRecordString(raw.instantiates, "project", `templates/${id}/template.yaml`);
  const pipelineRef = requiredRecordString(raw.instantiates, "pipeline", `templates/${id}/template.yaml`);
  const projectPath = safeResolve(workspaceRoot, path.resolve(templateDir, projectRef));
  const pipelinePath = safeResolve(workspaceRoot, path.resolve(templateDir, pipelineRef));
  if (!(await pathExists(projectPath)) || !(await pathExists(pipelinePath))) {
    throw new WorkspaceError(`Template ${id} referencia arquivos inexistentes.`, 422);
  }
  return {
    id,
    ...
    validationCase: raw.validationCase === true,
    templateDir,
    projectPath,
    pipelinePath,
  };
...
async function instantiateProjectTemplate(template: ProjectTemplateDescriptor, input: ReturnType<typeof normalizeCreateProjectInput>) {
  const sourceProject = parseMLOpsProject(YAML.parse(await readFile(template.projectPath, "utf-8")));
  const sourcePipeline = parsePipelineFlow(JSON.parse(await readFile(template.pipelinePath, "utf-8")) as unknown);
  return {
    sourceRoot: path.dirname(template.projectPath),
    project: parseMLOpsProject({ ...sourceProject, id: input.id, name: input.name }),
    pipeline: parsePipelineFlow({ ...sourcePipeline, id: `${input.id}_pipeline`, name: `${input.name} Pipeline` }),
  };
}

async function copyProjectTemplateAssets(sourceRoot: string, targetRoot: string): Promise<void> {
  for (const directory of ["data", "code"]) {
    const source = path.join(sourceRoot, directory);
    if (await pathExists(source)) {
      await cp(source, path.join(targetRoot, directory), { recursive: true, force: true });
    }
  }
}

function requiredRecordString(record: Record<string, unknown>, fieldName: string, source: string): string {
  const value = record[fieldName];
  if (typeof value !== "string" || !value.trim()) {
    throw new WorkspaceError(`${source}: ${fieldName} deve ser string não vazia.`, 422);
  }
  return value.trim();
}
...
async function createProject(workspaceRoot: string, body: CreateProjectBody) {
  const root = normalizeWorkspaceRoot(workspaceRoot);
  const template = body.templateId?.trim() ? await loadProjectTemplate(root, body.templateId) : null;
  let input = normalizeCreateProjectInput({ ...body, name: body.name?.trim() || template?.name });
  let projectRoot = safeResolve(root, `projects/${input.id}`);
  if (await pathExists(projectRoot)) {
    if (body.id?.trim()) {
      throw new WorkspaceError(`Projeto já existe: ${input.id}`, 409);
    }
    input = { ...input, id: await nextAvailableProjectId(root, input.id) };
    projectRoot = safeResolve(root, `projects/${input.id}`);
  }
  const templateBundle = template ? await instantiateProjectTemplate(template, input) : null;
  const project = templateBundle?.project ?? starterProject(input);
  const pipeline = templateBundle?.pipeline ?? starterPipeline(project);
  ...
  try {
    if (templateBundle) {
      await copyProjectTemplateAssets(templateBundle.sourceRoot, tempDir);
    }
...
async function importRuntimeProject(workspaceRoot: string, body: ImportRuntimeBody) {
  ...
  try {
    ...
    const quarantinedImport = Boolean(source.sourceGitUrl || source.sourceZip);
    const parsedSourceProject = parseMLOpsProject(parsedProjectYaml);
    const sourceProject = quarantinedImport ? quarantineExternalProject(parsedSourceProject) : parsedSourceProject;
    ...
    const parsedPipeline = parsePipelineFlow(parsedPipelineJson);
    const pipeline = quarantinedImport ? quarantineExternalPipeline(parsedPipeline) : parsedPipeline;
    ...
    await mkdir(tempDir, { recursive: true });
    try {
      if (importSource.mlopsDir && !quarantinedImport) {
      ...
      if (importSource.artifactsDir && !quarantinedImport) {
      ...
      if (importSource.customCodeDir && importSource.sourceKind === "app_metadata" && !quarantinedImport) {
        await copyAppMetadataCustomCode(importSource.customCodeDir, tempDir, pipeline);
      } else if (importSource.customCodeDir && !quarantinedImport) {
      ...
      if (!importSource.mlopsDir || quarantinedImport) {
        ...
        if (importSource.latestTrainingPath && !quarantinedImport) {
          await cp(importSource.latestTrainingPath, path.join(tempDir, ".mlops", "latest-training-result.json"), { force: true });
        }
      }
      if (quarantinedImport) {
        await writeFile(path.join(tempDir, ".mlops", "import-security.json"), `${JSON.stringify({
          source: source.sourceGitUrl ?? source.sourceZip,
          importedAt: new Date().toISOString(),
          policy: source.sourceGitUrl ? "external_git_quarantine" : "archive_quarantine",
          executablePythonReplaced: true,
          artifactsCopied: false,
          customCodeCopied: false,
          trainingStateCopied: false,
        }, null, 2)}\n`, "utf-8");
      }
...
function safeResolveStorePath(storeRoot: string, targetPath: string): string {
  ...
  assertNoSymlinkEscape(root, resolved, `Caminho fora do storage de snapshots por link simbólico: ${targetPath}`);
  return resolved;
}
...
async function runProcess(command: string, args: string[], workspaceRoot: string, timeoutMs: number, envOverrides: Record<string, string> = {}): Promise<{ exitCode: number | null; signal: NodeJS.Signals | null; stdout: string; stderr: string; timedOut: boolean }> {
  return new Promise((resolve, reject) => {
    ...
    child.stdout.on("data", (chunk) => {
      stdout = appendOutputTail(stdout, chunk, maxOutputBytes);
    });
    child.stderr.on("data", (chunk) => {
      stderr = appendOutputTail(stderr, chunk, maxOutputBytes);
    });
...
function appendOutputTail(current: string, chunk: string, maxBytes: number): string {
  const next = current + chunk;
  return next.length > maxBytes ? `[output truncated]\n${next.slice(-maxBytes)}` : next;
}
...
async function saveProject(workspaceRoot: string, projectId: string, body: unknown) {
  ...
  await atomicWriteText(safeResolve(workspaceRoot, loaded.projectPath), YAML.stringify(project));
...
async function savePipeline(workspaceRoot: string, projectId: string, body: unknown) {
  ...
  await atomicWriteText(safeResolve(workspaceRoot, loaded.pipelinePath), `${JSON.stringify(pipeline, null, 2)}\n`);
  return { status: "ok", pipelinePath: loaded.pipelinePath, pipeline };
}

async function saveProjectBundle(workspaceRoot: string, projectId: string, body: SaveProjectBundleBody) {
  if (body.project === undefined || body.pipeline === undefined) {
    throw new WorkspaceError("O bundle exige project e pipeline.", 422);
  }
  const loaded = await loadProjectBundle(workspaceRoot, projectId);
  const project = parseMLOpsProject(body.project);
  const pipeline = parsePipelineFlow(body.pipeline);
  if (project.id !== loaded.project.id) {
    throw new WorkspaceError(`O id do projeto enviado (${project.id}) não pode divergir da rota (${loaded.project.id}).`, 409);
  }
  if (normalizeArtifactRelativePath(project.pipelineRef) !== normalizeArtifactRelativePath(path.relative(loaded.projectRoot, safeResolve(workspaceRoot, loaded.pipelinePath)))) {
    throw new WorkspaceError("pipelineRef não pode mover o pipeline durante uma gravação de bundle.", 409);
  }

  const projectPath = safeResolve(workspaceRoot, loaded.projectPath);
  const pipelinePath = safeResolve(workspaceRoot, loaded.pipelinePath);
  const previousProject = await readFile(projectPath);
  const previousPipeline = await readFile(pipelinePath);
  try {
    await atomicWriteText(projectPath, YAML.stringify(project));
    await atomicWriteText(pipelinePath, `${JSON.stringify(pipeline, null, 2)}\n`);
  } catch (error) {
    await Promise.allSettled([
      atomicWriteBuffer(projectPath, previousProject),
      atomicWriteBuffer(pipelinePath, previousPipeline),
    ]);
    throw new WorkspaceError("Não foi possível gravar o bundle; os arquivos anteriores foram restaurados.", 500, error);
  }
  return { status: "ok", projectPath: loaded.projectPath, pipelinePath: loaded.pipelinePath, project, pipeline };
}

async function atomicWriteText(targetPath: string, content: string): Promise<void> {
  await atomicWriteBuffer(targetPath, Buffer.from(content, "utf-8"));
}
...
async function readGeneratedArtifactFile(workspaceRoot: string, outDir: string, filePath: string) {
  ...
  const listedFile = (await collectGeneratedArtifactFiles(root.absoluteOutDir)).find((file) => file.path === normalized);
  if (!listedFile) {
    throw new WorkspaceError(`Arquivo não disponível para leitura: ${normalized}`, 404);
  }
  const absolutePath = safeResolveArtifactFile(root.absoluteOutDir, normalized);
  const maxBytes = 512 * 1024;
  const handle = await open(absolutePath, "r");
  let bytesRead = 0;
  const buffer = Buffer.alloc(Math.min(listedFile.sizeBytes, maxBytes) + (listedFile.sizeBytes > maxBytes ? 1 : 0));
  try {
  ...
  return {
    ...
    path: normalized,
    content,
    sizeBytes: listedFile.sizeBytes,
    truncated: listedFile.sizeBytes > maxBytes,
  };
}
...
async function dockerRuntimeStatus(workspaceRoot: string, outDir: string): Promise<Record<string, unknown>> {
  ...
  if (docker.available && root.exists && root.composeExists) {
    const secrets = await ensureRuntimeSecrets(workspaceRoot, root.relativeOutDir);
    const ps = await runProcess("docker", dockerComposeArgs(root, ["ps", "--format", "json"]), workspaceRoot, 15_000, runtimeSecretsEnvironment(secrets)).catch((error) => {
      if (error instanceof WorkspaceError) {
...
async function runDockerRuntimeCommand(workspaceRoot: string, body: RuntimeDockerBody, action: "build" | "up" | "down"): Promise<Record<string, unknown>> {
  ...
  const started = performance.now();
  const result = await runProcess("docker", args, workspaceRoot, timeout, runtimeSecretsEnvironment(secrets));
  const historyEntry = await appendDockerRuntimeHistory(workspaceRoot, {
    ...
    stdout: redactRuntimeSecrets(trimProcessOutput(result.stdout), secrets),
    stderr: redactRuntimeSecrets(trimProcessOutput(result.stderr), secrets),
  });
  if (result.exitCode !== 0 || result.timedOut) {
    throw new WorkspaceError(result.timedOut ? `Docker ${action} excedeu o timeout.` : `Docker ${action} falhou.`, result.timedOut ? 504 : 500, {
      ...result,
      stdout: redactRuntimeSecrets(result.stdout, secrets),
      stderr: redactRuntimeSecrets(result.stderr, secrets),
      historyEntry,
    });
  }
...
async function dockerRuntimeLogs(workspaceRoot: string, outDir: string, tail: number): Promise<Record<string, unknown>> {
  ...
  const startedAt = new Date();
  const started = performance.now();
  const result = await runProcess("docker", args, workspaceRoot, 60_000, runtimeSecretsEnvironment(secrets));
  const historyEntry = await appendDockerRuntimeHistory(workspaceRoot, {
    id: randomUUID(),
    action: "logs",
    outDir: root.relativeOutDir,
  ...
  if (result.exitCode !== 0 || result.timedOut) {
    throw new WorkspaceError(result.timedOut ? "Docker logs excedeu o timeout." : "Docker logs falhou.", result.timedOut ? 504 : 500, {
      ...result,
      stdout: redactRuntimeSecrets(result.stdout, secrets),
      stderr: redactRuntimeSecrets(result.stderr, secrets),
      historyEntry,
...
async function dockerRuntimeInspect(workspaceRoot: string, outDir: string): Promise<Record<string, unknown>> {
  const root = await resolveGeneratedRuntimeRoot(workspaceRoot, outDir, false);
  const secrets = root.exists && root.composeExists ? await ensureRuntimeSecrets(workspaceRoot, root.relativeOutDir) : null;
  const docker = await dockerAvailability(workspaceRoot);
  const startedAt = new Date();
  const started = performance.now();
  ...
  if (docker.available && root.exists && root.composeExists) {
    const config = await runProcess("docker", dockerComposeArgs(root, ["config"]), workspaceRoot, 60_000, runtimeSecretsEnvironment(secrets as RuntimeSecrets)).catch((error) => processFailure(error));
    composeConfig = {
      ok: config.exitCode === 0 && !config.timedOut,
      command: `docker ${dockerComposeArgs(root, ["config"]).join(" ")}`,
      stdout: redactRuntimeSecrets(trimProcessOutput(config.stdout), secrets as RuntimeSecrets),
      stderr: redactRuntimeSecrets(trimProcessOutput(config.stderr), secrets as RuntimeSecrets),
      timedOut: config.timedOut,
    };
    const images = await runProcess("docker", dockerComposeArgs(root, ["images", "--format", "json"]), workspaceRoot, 60_000, runtimeSecretsEnvironment(secrets as RuntimeSecrets)).catch((error) => processFailure(error));
    composeImages = {
...
async function smokeRuntime(baseUrl?: string, payload?: Record<string, unknown>, timeoutMs?: number, runtimeApiKey?: string): Promise<Record<string, unknown>> {
  ...
  ) => performSmokeRuntimeCheck(checkBase, name, method, pathName, requestBody, requestTimeoutMs, validate, runtimeApiKey);
  const checks = [];
  ...
  const registryDetail = await smokeRuntimeCheck(base, "model_registry_detail", "GET", `/models/${encodeURIComponent(smokeModelId)}`, undefined, checkTimeout, (body) => isRecord(body) && body.registered === true);
  const candidateAlreadyRegistered = isRecord(registryDetail.rawBody) && registryDetail.rawBody.registered === true;
  if (!candidateAlreadyRegistered) {
    const registrationCheck = await smokeRuntimeCheck(base, "model_registration", "POST", "/models/register", { confirm: true, model_id: smokeModelId, algorithm: "smoke_candidate", status: "candidate", requested_by: "smoke" }, checkTimeout, (body) => isRecord(body) && isRecord(body.model) && body.model.id === smokeModelId);
...
async function performSmokeRuntimeCheck(
  base: URL,
  name: string,
  method: "GET" | "POST",
  pathName: string,
...
async function ensureRuntimeSecrets(workspaceRoot: string, outDir: string): Promise<RuntimeSecrets> {
  const root = await resolveGeneratedRuntimeRoot(workspaceRoot, outDir, false);
  const secretsPath = runtimeSecretsPath(workspaceRoot, root.relativeOutDir);
  try {
    const parsed = JSON.parse(await readFile(secretsPath, "utf-8")) as unknown;
    if (!isRecord(parsed) || typeof parsed.runtimeApiKey !== "string" || parsed.runtimeApiKey.length < 24 || typeof parsed.postgresPassword !== "string" || parsed.postgresPassword.length < 24) {
      throw new WorkspaceError(`Arquivo de segredos do runtime inválido: ${toWorkspaceRelative(workspaceRoot, secretsPath)}`, 500);
    }
    return { runtimeApiKey: parsed.runtimeApiKey, postgresPassword: parsed.postgresPassword };
  } catch (error) {
  ...
  const secrets: RuntimeSecrets = {
    runtimeApiKey: randomBytes(32).toString("base64url"),
...
function runtimeSecretsPath(workspaceRoot: string, outDir: string): string {
  const fileName = `${Buffer.from(outDir).toString("base64url")}.json`;
  return safeResolve(workspaceRoot, path.join(".mlops-studio", "runtime-secrets", fileName));
}

function runtimeSecretsEnvironment(secrets: RuntimeSecrets): Record<string, string> {
  return {
    MLOPS_RUNTIME_API_KEY: secrets.runtimeApiKey,
    POSTGRES_PASSWORD: secrets.postgresPassword,
  };
}

function redactRuntimeSecrets(value: string, secrets: RuntimeSecrets): string {
  return [secrets.runtimeApiKey, secrets.postgresPassword].reduce(
...
async function collectGeneratedArtifactFiles(artifactRoot: string, currentDir = "", result: Array<{ path: string; sizeBytes: number }> = []) {
  ...
  for (const entry of entries) {
    ...
    if (!entry.isFile() || isSensitiveArtifactPath(relativePath) || [".pyc", ".pyo", ".db", ".sqlite", ".sqlite3"].includes(path.extname(entry.name))) {
      continue;
    }
...
function isSensitiveArtifactPath(relativePath: string): boolean {
  const baseName = path.basename(relativePath).toLowerCase();
  if (baseName.startsWith(".env") && baseName !== ".env.example") {
    return true;
  }
  if ([".npmrc", ".pypirc", "credentials.json", "credential.json", "secrets.json", "secret.json"].includes(baseName)) {
    return true;
  }
  return [".pem", ".key", ".p12", ".pfx"].includes(path.extname(baseName));
}
...
function safeResolveArtifactFile(artifactRoot: string, filePath: string): string {
  ...
  assertNoSymlinkEscape(root, resolved, `Arquivo fora do artefato gerado por link simbólico: ${filePath}`);
  return resolved;
}
...
async function nextAvailableProjectId(workspaceRoot: string, baseId: string): Promise<string> {
  for (let suffix = 2; suffix <= 10_000; suffix += 1) {
    const candidate = `${baseId}-${suffix}`;
    if (!(await pathExists(safeResolve(workspaceRoot, `projects/${candidate}`)))) {
      return candidate;
    }
  }
  throw new WorkspaceError(`Não foi possível reservar um id livre para ${baseId}.`, 409);
}

function quarantineExternalProject(project: MLOpsProject): MLOpsProject {
  return parseMLOpsProject(quarantineExecutableValue(project));
}

function quarantineExternalPipeline(pipeline: PipelineFlow): PipelineFlow {
  return parsePipelineFlow(quarantineExecutableValue(pipeline));
}

function quarantineExecutableValue(value: unknown): unknown {
  if (Array.isArray(value)) {
    return value.map((item) => quarantineExecutableValue(item));
  }
  if (!isRecord(value)) {
    return value;
  }
  ...
  for (const [key, nestedValue] of Object.entries(value)) {
    if (key === "python" && isRecord(nestedValue)) {
      const safePython: Record<string, unknown> = {
        ...nestedValue,
        codeInline: [
          "def run(input: dict, context: dict) -> dict:",
          "    raise RuntimeError('Bloco Python em quarentena: revise e substitua o código antes de executar.')",
          "",
        ].join("\n"),
        dependencies: [],
        networkPolicy: "none",
      ...
      delete safePython.codePath;
      result[key] = safePython;
      continue;
    }
    result[key] = quarantineExecutableValue(nestedValue);
  }
  return result;
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  const authToken = process.env.MLOPS_STUDIO_API_TOKEN?.trim() || randomBytes(32).toString("base64url");
  if (!process.env.MLOPS_STUDIO_API_TOKEN) {
    console.warn(`[control-api] MLOPS_STUDIO_API_TOKEN não definido; token efêmero gerado: ${authToken}`);
  }
  const app = buildApp({ logger: true, authToken });
